...
class Human:
    ...
    def live(self, day):
        ...
        if self.car is None:
                self.get_car()
                print(f"i not loh {self.car.brand}")
        if self.job is None:
                    self.get_job()
                    print(f"iget job - {self.job.job}")
        self.days_index(day)
        ...
        if dice == 1:
            ...
            print("cleaning idi v popu")
            self.clean_house()
        elif dice == 3:
            print("go to svoy work")
            self.work()
        elif dice == 4:
...
brands_of_car = {
    "bmw": {"fuel": 99, "strength": 100, "consumption": 6},
